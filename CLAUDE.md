...
**昇降・カメラ制御・安全設計・th-system との結合・アーキテクチャ全般について、ユーザーから方針変更の指示があった場合は、コードを修正する前に必ず `docs/plan/spec/` の該当箇所を更新すること。** コード修正はその後に行う。spec とコードの内容に矛盾が生じた場合は、ユーザーに確認しどちらが正か明確にしてから作業を進める。
...
- **th-system のリポジトリはここから編集しない。**別リポジトリで、別の正本と運用ルールを持つ。
  結合に関わる変更が th-system 側に要るときは、差分案を示してユーザーに渡す。
- th-system 側の関連箇所（`O-a1` 位置決め許容差・`O-a6` 正式インターフェース・`AT_PANEL`）の
  写しは [Spec-open.md](docs/plan/spec/Spec-open.md) §1 にある。**写しより th-system の原文が正。**
  食い違いに気づいたら写しを直す。
...
- 設計段階の今は「ビルドとテスト」「アーキテクチャ」の節が無い。**実装の構成（ROS2 か単体ファームか等。`Spec-open.md` §2）が決まったら足す。**
