...
const states = ['', '?h=1100&ceil=stale&hold=lift_up', '?settings=1', '?mock=1'];
(async () => {
  ...
  for (const [w,h] of sizes) for (const st of states) {
    ...
    const r = await p.evaluate(() => {
      ...
      const must = [...document.querySelectorAll('.hold, input[type=range], #openSettings')];
      ...
      for (const el of must) {
        ...
        const box = el.closest('.card, .sheet, .mockpop');
